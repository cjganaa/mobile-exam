      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.homeworkCard}>
            <TouchableOpacity style={styles.progressContainer} onPress={() => handleGradePress(item)}>
              <CircularProgress
                size={50}
                width={5}
            ...
            <TouchableOpacity style={styles.homeworkTextContainer} onPress={() => handleAttendancePress(item)}>
              <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: 220 }}>
              ...
              <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>
                ...
                <View style={{ width: 200, height: 10, backgroundColor: "#ddd", borderRadius: 5, flex: 1, flexDirection: "row", overflow: 'hidden' }}>
                  <View style={{ width: `${(attendanceData.filter(att => att.subject === item.subject && att.present).length / attendanceData.filter(att => att.subject === item.subject).length * 100 || 0)}%`, height: "100%", backgroundColor: "#00ce90",borderRadius:400 }} />
                </View>
              </View>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  homeworkCard: { flexDirection: "row", alignItems: "center", backgroundColor: "#F3F4F6", padding: 12, borderRadius: 10, marginBottom: 8, marginHorizontal: 16 },
